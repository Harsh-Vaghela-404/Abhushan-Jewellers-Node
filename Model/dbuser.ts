...
export class dbUser extends db{
     ...
     */
    async Signup(first_name:string, last_name:string, second_name:string, contact:string, address:string, role_id:string, password:string){
        //Hashed Password to overcome the data security of users
        let return_data:any = {...return_without_data}
        
        //check user exsist

        let check_user = await this.getUser(0, contact)
    ...
    async Login(contact:string, password:string){
        let return_data:any = {...return_without_data}
        let user_data:any = await this.getUser(0, contact)
        //check user exsist
     ...
     */
    async getUser(id:number = 0, contact:string){
        let return_data:any = {...return_with_data}
        let user_data:any = []
        if(id){ 
            user_data = await this.selectRecord(id,"*")
        }else{ // give all user data

            if(contact){ // find user according to the condition
                this.where = `WHERE contact = `+ contact;
            }
            user_data = await this.allRecords("*")
        }
